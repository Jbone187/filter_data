...
function pullData(newArr) {
    ...
    //This code allows script to search for .json files but theres a bug the doesn't allow import of .json file that ran at 000 hour 12am
...
*/
    ...
    let crush_json = require(`${crush_args[3]}/${crush_args[2]}${numArray}.json`);
    ...
    if (crush_args[5] === 'false') {
        ...
        fs.unlink(`${crush_args[3]}/${crush_args[2]}${numArray}.json`, (err) => {
            if (err) {
                throw err;
            }

            console.log(`${crush_args[2]}${numArray}.json File has been deleted...`);
        });

    } else if (crush_args[5] === 'true') {

        fs.copyFileSync(`${crush_args[3]}/${crush_args[2]}${numArray}.json`, `${crush_args[6]}/${crush_args[2]}${date}${numArray}.json`)
        ...
        console.log(`${crush_args[2]}${date}${numArray}.json File has been moved to file directory.. ${crush_args[6]}`);
